...
4. 文章词汇量估算：输入英文文章后，系统返回估算词汇量、教育阶段、difficulty 尾部加权值、覆盖率、内容词数、唯一词数和未匹配词样例。
...
在 v1 清洗词库上，尾部加权公式把 C/F/P/K 分别估计为 6,399、5,021、3,084、1,145，排序仍满足 C > F > P > K，且比旧中位数法更能体现 C、F 文本的高难词尾部信号。扩展后 v2 词库的 C 类语料 token 覆盖率达到 93.7%，F 类达到 94.0%；`algorithmic`、`blockchain`、`governance`、`mitigation`、`biodiversity` 等高难信号词进入词库后，估算基础更可靠。
...
项目不足也比较明确：第一，缺少真实学生 CET-4/6 或标准化考试成绩的交叉验证；第二，v2 选择题测验虽然已支持切换，但仍需要更多真实答题数据验证其对用户体验和估算量表的影响；第三，文章估算的 p75/p95 权重仍需更多真实阅读材料校验；第四，专有名词、缩写和连字符复合词的处理仍有改进空间。
